...
class Admin extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['order'] = $this->General_model->count('tx_order');
        $data['rating_order'] = $this->General_model->count_avg("rating", "tx_order_rating");
        $data['total_trx_order'] = $this->General_model->count_sum("grand_total_amount", "tx_order");
        $data['total_revenue'] = $this->General_model->count_sum("revenue", "tx_order");
        ...
        foreach ($popular_ingredients as $recipe) {
		  ...
		  foreach ($valuee['items'] as $itemzz) {
		      $ingArray[] = trim(strtolower($itemzz['title']));
	...
	$ingArray = array();
        foreach ($popular_ingredients as $recipe) {
            ...
            foreach ($result as $keys => $valuess) {
              foreach ($valuess['items'] as $items) {
                  $ingArray[] = trim(strtolower($items['title']));
        ...
        // $ii=0;
        // foreach ($dataah_mpasi as $key_mpasii => $value_mpasii {
        //     //echo $key1.'---'.$value1.'<br>';
        //     $data_inputs = array('grafik_mpasi' => $key_mpasii, 'grafik_mpasi_jum' => $value_mpasii;
        //     $insert_ingredient_mpasi = $this->General_model->insert('grafik_ingredients_mpasi', $data_inputs);
    ...
    public function customerdetail_real($customer_id)
    {
        ...
        $data['list_orders'] = $this->General_model->get_join_wheres("tx_order", "m_recipe", "tx_order.recipe_id = m_recipe.recipe_id", "tx_order.customer_id = $customer_id");
